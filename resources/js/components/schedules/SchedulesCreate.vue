<template>
    ...
    <div class="w-full px-4 py-6 bg-white text-gray-900 drop-shadow md:w-3/4 md:px-8 md:py-8 lg:w-3/6 mx-auto">
        ...
        <form @submit.prevent="createSchedule">
            ...
            <!-- Movies -->
            <div class="mb-4 grow">
                <label class="text-gray-900">Movie</label>
                <div class="mt-2">
                    <select v-model="movie_id" @change="getDates" class="w-full border border-gray-300 text-gray-800 focus:outline-none px-3 py-2 capitalize" required>
                        <option value="" selected disabled class="lowercase">Select a movie</option>
                        <option v-for="movie in movies" :key="movie.id" :value="movie.id" :disabled="new Date(currentDate) > new Date(movie.end_date) ? true : false">
                            {{ movie.title }} 
                            <span v-if="new Date(currentDate) > new Date(movie.end_date) ? true : false">(ended)</span>
                        </option>
                    </select>
                </div>
            </div>
            ...
            <!-- Cinemas -->
            <div class="mb-4 grow">
                <label class="text-gray-900">Cinema</label>
                <div class="mt-2">
                    <select v-model="cinema_id" class="w-full border border-gray-300 text-gray-800 focus:outline-none px-3 py-2 capitalize" required>
                        <option value="" selected disabled class="lowercase">Select a cinema</option>
                        <option v-for="cinema in cinemas" :key="cinema.id" :value="cinema.id" :date-extra="cinema.seating_capacity">{{ cinema.cinema_name }}</option>
                    </select>
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row gap-x-3">
                ...
                <div class="mb-4 grow">
                    ...
                    <div class="mt-2">
                        <input id="date" type="date" v-model="date" :min="new Date(showing_date) < new Date(currentDate) ? currentDate : showing_date" :max="end_date" :class="!movie_id ? 'cursor-not-allowed' : ''" class="w-full border border-gray-300 text-gray-800 focus:outline-none px-3 py-2" :disabled="!movie_id ? true : false" required/>
                    </div>
                </div>
...
export default {
    ...
    async mounted() {
        // Get all movies
        await axios.get('/admin/movies/titleAscending')
        .then(response => {
            console.log(response.data);
        ...
        // Get all cinemas
        await axios.get('/admin/cinemas/nameAscending')
        .then(response => {
            console.log(response.data);
