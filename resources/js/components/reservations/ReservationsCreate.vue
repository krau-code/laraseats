<template>
    <title>Add reservation | LaraSeats Admin</title>

    <div class="w-full px-4 py-6 bg-white text-gray-900 drop-shadow md:w-3/4 md:px-8 md:py-8 lg:w-3/6 mx-auto">
        <!-- Heading -->
        <div class="flex justify-between items-center mb-4">
            <h1 class="text-3xl font-bold">Add a reservation</h1>
            <router-link :to="{ name: 'reservations.pick' }" class="text-gray-700 hover:text-gray-600">Go back</router-link>
        </div>
        ...
        <form @submit.prevent="createReservation">
            ...
            <div class="mb-4 grow">
                ...
                <div class="mt-2">
                    <input id="showing_date" type="date" v-model="date" class="w-full border border-gray-300 text-gray-400 focus:outline-none px-3 py-2 bg-gray-100 cursor-not-allowed" disabled required/>
                </div>
            </div>
            ...
            <div class="mb-4 grow">
                ...
                <div class="mt-2">
                    <input id="showing_date" type="text" v-model="title" class="w-full border border-gray-300 text-gray-400 focus:outline-none px-3 py-2 bg-gray-100 capitalize cursor-not-allowed" disabled required/>
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row gap-x-3">
                ...
                <div class="mb-4 grow">
                    ...
                    <div class="mt-2">
                        <input type="text" v-model="cinema_name" class="w-full border border-gray-300 text-gray-400 focus:outline-none px-3 py-2 bg-gray-100 uppercase cursor-not-allowed" disabled required/>
                    </div>
                </div>
                ...
                <div class="mb-4 grow">
                    ...
                    <div class="mt-2">
                        <input type="number" ref="amount" v-model="amount" class="w-full border border-gray-300 text-gray-400 focus:outline-none px-3 py-2 bg-gray-100 cursor-not-allowed" disabled required/>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row gap-x-3">
                <!-- Quantity -->
                <div class="mb-4 grow">
                    <label for="quantity" class="text-gray-900">How many seats?</label>
                    <div class="mt-2">
                        <input id="quantity" type="number" v-model="quantity" min="1" :max="seat_left >= 10 ? 10 : seat_left" class="w-full border border-gray-300 text-gray-800 focus:outline-none px-3 py-2" required/>
                    </div>
                ...
                <div class="mb-4 grow">
                    ...
                    <div class="mt-2">
                        <input  id="amount" ref="total" type="number" v-model="totalAmount" class="w-full border border-gray-300 text-gray-500 focus:outline-none px-3 py-2 bg-gray-100 cursor-not-allowed" disabled required/>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        async createReservation() {
            ...
            await axios.post('/admin/reservations/create', data)
            .then(response => {
                ...
                axios.post('/admin/schedules/updateSeat/' + this.id, updateSeat)
                .then(res => {
    ...
    async mounted() {
        // Get single schedule
        await axios.get('/admin/schedules/'+this.id)
        .then(response => {
            console.log(response.data);
